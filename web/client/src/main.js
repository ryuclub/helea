...
// 三族角色资源(按 race+sex 加载对应进阶精灵包 + 重映射动作下标), 带缓存。复刻开源客户端逻辑。
// race: "slayer"|"vampire"|"ousters"; sex: 0 男 / 1 女。返回 {frames, anim}。
const _raceAssetCache = new Map(); const _raceInflight = new Map();
...
// 收集角色多部位 FrameID(身体 + 各装备外观层, 按绘制顺序叠)。gearInfo: [{itemClass,itemType,slotID}]|null。
// 装备外观帧来自官方 Item.inf 的 AddonMale/FemaleFrameID(iteminfo.js getItemInfo); FrameID 落在 AC 包部位层(0~14)。
...
async function buildRaceAssets(race, sex, gearInfo = null) {
  ...
  const p = (async () => {
    ...
    log(`角色资源就绪: ${pack} (${parts.length}部位 ${ids.size}精灵)`, "info");
...
let _creatureCFPK = null; const _creatureCache = new Map(); const _creatureInflight = new Map();
async function loadCreatureAssets(id, isNPC = false) {
...
// 生物高度表(Creature.inf 抽取): h[creatureType]=头顶血条/名字定位用 Height(忠实开源 GetHeight, 见 memory)。
// 怪物 c.spriteType 即包里 MonsterType, 直接索引 Creature.inf(开源 SetCreatureType(monsterType))。玩家按种族/性别名查。
let creatureInfo = null;
async function ensureCreatureInfo() {
...
function playerHeight(race, sex) { const nm = (PLAYER_CREATURE_NAME[race] || [])[sex ? 1 : 0]; const r = creatureInfo && creatureInfo.byName && creatureInfo.byName[nm]; return r ? r.h : 75; }
...
async function addCreature(c) {
  ...
  try {
    ...
    const ch = creatureInfo && creatureInfo.h && creatureInfo.h[c.spriteType];   // Creature.inf[MonsterType].Height
