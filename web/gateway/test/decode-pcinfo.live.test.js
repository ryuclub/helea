...
function conn(port) {
  ...
  ws.on("message", (d) => fr.push(new Uint8Array(d))); return ws; // node ws: 直接传 data
}
...
// 整流程重试(抗间歇性 LG 握手竞态): 每次新连接, 直到拿到 GC_UPDATE_INFO
async function once() {
  ...
  const raw = game.raw["GC_UPDATE_INFO"]; game.close();
  return raw || null;
}
let raw = null;
for (let attempt = 1; attempt <= 6 && !raw; attempt++) {
  raw = await once();
  console.log(`尝试 ${attempt}: ${raw ? "✅ 拿到 GC_UPDATE_INFO" : "未成功, 重试…"}`);
  if (!raw) await sleep(1200);
}
if (!raw) { console.log("❌ 6 次重试仍无 GC_UPDATE_INFO"); process.exit(1); }
